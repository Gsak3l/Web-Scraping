    ...
    def follow_random_users(self):
        ...
        while flag:
            if len(verification_code) == 3:
                ...
                random_number = randint(5, 20)  # random number of accounts the bot will follow
                for i in range(1, random_number):
                    try:
                        # random accounts that the bot follows
...
class Sign_Up_Bot:
    ...
    def get_bio(self, bio_list):
        ...
        time.sleep(1)
        # cookie stuff
        bot.find_element_by_xpath('//*[@id="onesignal-slidedown-cancel-button"]').click()
        time.sleep(0.5)
        # generating a bio
...
if __name__ == '__main__':
    ...
    process10 = multiprocessing.Process(target=bot10.tweet_inspirational_quote, )
